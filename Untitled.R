...
make_data_chunks <- function(chunksize) {
  # Split year range into chunks of 10 years each
  ...
  for (chunk in yearchunks) {
    ...
    #dataname <- paste0('target_signif', paste(chunk, collapse = '_'))
    dataname <- paste(c(head(chunk, 1), tail(chunk, 1)), collapse = "-")
    #assign(dataname, target_signif, envir = .GlobalEnv)
    resultlist[[dataname]] <- target_signif
  }
  assign("signifdata", resultlist, envir = .GlobalEnv)
  save(signifdata, file = "/Users/phb514/mygit/memo-keywords-across-time/rdata/signifdata.Rda")
}
...
get_year_freqlist <- function(year) {
  rdatadir <- '/Users/phb514/mygit/memo-keywords-across-time/rdata/'
...
get_corpus <- function(pattern) {
  srcdir = '/Users/phb514/mygit/memo-keywords-across-time/corpustxts'
